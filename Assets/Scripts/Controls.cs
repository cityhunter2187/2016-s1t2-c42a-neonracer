...
public class Controls : MonoBehaviour {
    ...
    void Start() {
        ...
        item = GameObject.Find("Player").GetComponent<itemPickup>();
    ...
    void Movement()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Z))
        {
            if(item.hasItem == true) {
                ...
                {
                    GameObject newShell = Instantiate(item.items[0], new Vector3(transform.position.x, transform.position.y + 0.8f, transform.position.z), Quaternion.identity) as GameObject;
